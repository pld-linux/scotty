...
Scotty jest rozszerzeniem Tcl do budowania aplikacji do zarządzania
siecią przy użyciu Tcl (i Tk). Scotty udostępnia nowe polecenia Tcl
do:
- wysyłania i odbierania pakietów ICMP
- odpytywania DNS
- dostępu do gniazdek UDP
- testowania i używania niektórych Sun RPC
- ściągania i serwowania dokumentów po HTTP
- wysyłania i odbierania komunikatów SNMP (v1, v2C, v3)
- pisania agentów SNMP
- parsowania i dostępu do definicji SNMP MIB
- kolejkowania zadań do regularnego wykonywania
- realizacji programowania sterowanego zdarzeniami na mapach
  sieciowych

Ta dystrybucja Scotty zawiera także źródła Tkined. Jest to sieciowy
edytor pozwalający rysować mapy obrazujące konfigurację sieci.
Najważniejszą cechą Tkined jest programistyczny interfejs pozwalający
aplikacjom zarządzającym siecią rozszerzać jego możliwości. Większość
aplikacji dla Tkined jest pisana przy użyciu rozszerzenia Tnm dla Tcl.
